<%@ page import="com.ys.crm.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.ys.crm.workbench.domain.Tran" %>
    ...
    <script type="text/javascript">
        ...
        /*function changeIcon(stage, i) {
            ...
            if ("0" == currentPossibility) {
                ...
                //如果当前可能性不为0 前7个绿圈，绿色标记，黑圈，后两个一定是黑叉
            } else {
                //遍历前七个
                for (var i = 0; i < point; i++) {
                    if (i == index) {
...
<div style="position: relative; left: 40px; top: -50px;">
    ...
    <%
        ...
        //准备当前阶段可能性
        //String possibility = tran.getPossibility();
        String currentPossibility = pMap.get(stage);

        //判断当前阶段，如果可能性为0,前7个是一定是黑圈，后两个一个红叉，一个黑叉
        if ("0".equals(currentPossibility)) {
            for (int i = 0; i < stageList.size(); i++) {
                DicValue dicValue = stageList.get(i);
                String listStage = dicValue.getValue();
                String listPossibility = pMap.get(listStage);
                //如果遍历出来可能性为0，说明前7个一定是黑圈，后两个一个红叉，一个黑叉
    ...
    <%
        ...
        //如果当前阶段可能性不为0 ，前七个有绿圈，绿标记，黑圈，后两个一定是黑叉
    } else {
        //准备当前阶段的下标
        int index1 = 0;
        for (int i = 0; i < stageList.size(); i++) {
            String stage1 = stageList.get(i).getValue();
            if (pMap.get(stage1).equals(currentPossibility)) {
                index1 = i;
                break;
            }
        }
    ...
    <span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')"
          ...
          data-toggle="popover" data-placement="bottom"
          data-content="<%=dicValue.getText()%>" style="color: #00FF00;"></span>
    -----------
    <%
        //如果小于当前阶段
    } else if (i < index1) {
